...
class peak_detection(object):
    ...
    def __init__(self, signal, flat_method = 'l', height_type = 'local'):
        if signal is None:
    ...
    def findPeaks(self, minPeakDist = None, minPeakHeight = None, maxPeakHeight = None, \
                  minPeakTh = None, maxPeakTh = None, height_type = 'prominence', remove_trend = False):
    ...
    def _findHeight(self, locs, y):
        ...
        for i in range(1, len(aug_locs) - 1):
            ...
            #height[i - 1] = max(y[aug_locs[i]] - left.min(), y[aug_locs[i]] - right.min())
            #height[i - 1] = min(y[aug_locs[i]] - left.min(), y[aug_locs[i]] - right.min())
            # TODO: this definition depends on the problem at hand
            height[i - 1] = y[aug_locs[i]] - right.min()
